...
- Чистый ручной каталог: новая Германия (TCP/Hysteria2), Франция (TCP/Hysteria2), Финляндия (XHTTP/TCP/Hysteria2), затем будущая Польша, старая Германия только `TCP Reality` с пометкой `(резерв)`, последним финский LTE. Нумерацию `#1/#2` пользователям не показывать.
