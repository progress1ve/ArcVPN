...
- Happ может объединять или обновлять поверх друг друга разные subscription URL, если сервер отдаёт им одинаковый HTTP-заголовок `Profile-Title`. Поэтому персональная ссылка устройства обязана иметь уникальное название `ArcVPN • <устройство>`, master URL — `ArcVPN • СТАРАЯ ПОДПИСКА`, а служебные профили — `ArcVPN • ЛИМИТ УСТРОЙСТВ` / `ArcVPN • УСТРОЙСТВО УДАЛЕНО`. Иначе после успешного импорта Happ способен через несколько секунд обновить старый master URL и снова показать сообщение «требуется обновить подключение».
- Уточнение: master URL нельзя полностью блокировать WebApp-only сценарием — пользователь без работающего VPN может не открыть Telegram/WebApp. При прямом импорте master URL сервер создаёт один детерминированный управляемый слот `Подключение по ссылке` и отдаёт рабочий профиль `ArcVPN • Подключение по ссылке`. Этот слот учитывается в общем лимите; остальные устройства рекомендуется добавлять персональными ссылками через WebApp.
