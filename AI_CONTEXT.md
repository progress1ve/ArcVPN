...
- Причина неработавшего освобождения устройства: HTTPS-мост импорта регистрировал browser device token, но передавал Happ исходный URL подписки без `?device=<token>`. Теперь и автоматический переход, и fallback-кнопка передают стабильный token в URL. Сервер при обновлении может вернуть отозванному устройству служебный профиль вместо рабочих inbound. Старые импорты без token необходимо один раз удалить/переимпортировать после обновления; удалённую локальную конфигурацию на клиенте сервер отозвать задним числом не может.
