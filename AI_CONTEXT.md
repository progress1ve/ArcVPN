...
- Рекламный `start=ad_*` считается привлечением только если этот же `/start`
  создал нового пользователя. Уже существующий пользователь не получает
  campaign attribution/bonus и не попадает в счетчики, даже если впервые открыл
  конкретную рекламную ссылку.
- Оплаты кампании считаются только со времени attribution и только в успешных
  статусах. `revenue_cents` является нормализованным контрактом UI: связанная с
  YooKassa запись уже хранит копейки, старая несвязанная RUB-запись хранит целые
  рубли и умножается на 100 только при чтении; историю платежей не переписывать.
- Обязательная подписка на Telegram-канал перехватывает первый `/start` до
  обычного handler. Рекламный payload необходимо сохранить в FSM до показа gate,
  однократно извлечь в `check_subscribe` после создания пользователя и только
  затем сделать attribution/entry bonus. Нельзя снова полагаться только на
  `is_new` внутри обычного `/start`: такой путь теряет реальные регистрации.

## Admin catalog safety and Client 360 (2026-08-22)

- Идентичность профиля подписки определяется нормализованным source name без
  флага и декоративной молнии. Флаг страны вычисляется сервером и не может быть
  случайно удалён через админку; display name и sort order не создают Remnawave
  Host/inbound и не меняют UUID или URL действующих подписок.
- Каталог админки показывает только реально предусмотренные профили. Второй LTE
  нельзя рисовать как default: он появляется лишь после создания и проверки
  отдельного Remnawave Host/inbound/CDN-маршрута.
- Client 360 в админке объединяет точный срок, подписки, оплаты, баланс и
  устройства. Публичные subscription URL, provider payment IDs и другие секреты
  в этот API не возвращаются.
- Для независимо выбираемых DE LTE и NL LTE один общий Yandex CDN resource с
  одним origin недостаточен: нужны разные CDN resources/origin groups (и разные
  GSLB CNAME), либо нужно осознанно считать их одним общим LTE pool.
- Клиентский JSON-автовыбор называется `🇪🇺 Автовыбор | Самый быстрый`. Его
  основной balancer проверяет только обычные маршруты; LTE/CDN нельзя включать в
  observatory, иначе одни фоновые пробы расходуют тарифицируемый CDN-трафик.
- Несколько balancer-профилей не заменяют XHTTP/CDN-транспорт: балансировщик
  выбирает уже существующие outbounds, но сам по себе не создаёт обход глушения.
  Не публиковать псевдо-CDN-профили, которые фактически используют обычный TCP.
- Каталог v53 разделяет `enabled` (виден самостоятельной строкой) и
  `include_in_auto` (доступен как внутренний outbound JSON-балансировщика).
  Это позволяет скрывать служебные Remnawave Hosts из Happ, сохраняя их внутри
  синтетического автовыбора. Для plain/base64 скрытый Host не выдаётся.
...
- Commit `9a69ba1` removes the experimental anchor/wheel smoothing and `scroll-behavior: smooth`; all landing scrolling is native browser behavior. Cabinet image width/height attributes remain matched to the encoded assets.
- Commits `89b1595`, `d6c9613`, `c800338`, `5dfb6df` and final scrolling correction `9a69ba1` are deployed on the Poland control plane. Browser QA passed at 360x800, 390x844, 768x1024, 1280x900 and 1600x900 with no overflow; public landing, app, Hero media and cabinet images return HTTP 200.
...
- SMTP and legal operator fields are safe placeholders in `.env.example`; real secrets and legal requisites must be supplied only through production configuration. The public legal renderer suppresses unset placeholder values, but verified operator name, INN, OGRN/OGRNIP, address and legal email remain required for complete disclosure.
...
- WebApp account foundation added locally on 2026-07-29 (migration 27): verified email linking/login with hashed one-time codes and hashed 30-day sessions; notification preferences for expiry/traffic/first connection; import-time device registry. The public User Agreement and Privacy Policy were revised on 10 September 2026. Email delivery requires production `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM`, `SMTP_USE_TLS` environment values. Never store raw codes/session tokens.
- Legal consent version `2026-09-10` is canonical across the public page, WebApp and Telegram gate. Daily backup maintenance remains local-only: retained DB copies are created and expired copies cleaned. Both the Telegram backup archive and daily statistics report to admins are disabled.
...
- `get_user_primary_key()` обязан возвращать `sub_id`: от него зависят текстовая
  ...
  «Подключить VPN».
- Миграция v34 добавляет идемпотентные lifecycle-события. Спустя 24 часа после
  активации пробной подписки бот один раз просит оценку; старые уже отправленные
  `day5_rating`-кнопки остаются совместимыми, но повторно не рассылаются. Спустя 2–10 дней после окончания один раз
...
- Business Console показывает отправленные/полученные пробные оценки, средний
  балл, распределение 1/3/5 и последних ответивших. Bot Settings открывает
  существующий WebApp email-flow через `/app?screen=email`; возврат из выбора
  тарифа продления ведёт в список подписок (`my_keys`).
- В Client 360 есть отдельная вкладка «Ответы»: она показывает ответы конкретного
  пользователя на `trial_day1_rating`/совместимый `day5_rating` и
  `expired_winback`, включая свободный комментарий и время ответа. API детали
  пользователя возвращает их в `lifecycle_answers`; отсутствие таблицы в старой
  базе безопасно даёт пустой список.
...
- Исторический subscription-wide режим Happ `subscription-autoconnect: lowestdelay` отменён владельцем 2026-09-01: он переупорядочивает видимые локации по пингу. Каталог должен сохранять серверный порядок; автоподключение реализуется только отдельным первым JSON-профилем с внутренним Xray `leastLoad`. Внешний Happ Provider ID также полностью удалён из URL, тела и заголовков подписки; ArcVPN не должен зависеть от кабинета, статистики или платных функций Happ.
...
- Историческая привязка Happ Provider ID к `sub.arccnet.space` отменена владельцем 2026-09-01. Не возвращать `providerid` в import URL, комментарии или HTTP-заголовки. Не передавать `subscriptions-sort-type` и другие команды сортировки: видимый порядок задаётся только массивом JSON; отдельный Auto JSON-профиль отвечает за выбор по задержке.
- Subscription meta-параметры `subscription-autoconnect`, `subscription-autoconnect-type=lowestdelay` и `subscription-ping-onopen-enabled` больше не выдаются: видимые строки Happ нельзя сортировать по пингу. Отдельный первый JSON-профиль `Автовыбор | Самый быстрый` остаётся полноценной Xray-конфигурацией с `burstObservatory` и `leastLoad`; после него сервер фиксированно выдаёт YouTube, Эстонию, Нидерланды, Германию, `🇪🇺 Лучший обход` и обходы #2–#5.
...
- On 2026-08-22 public `remna.arccnet.space -> 217.60.33.38` DNS was restored, the temporary
  control-plane hosts-file override was initially removed. Intermittent local DNS/hairpin
  timeouts then caused partial minute-sync failures even though public Remnawave stayed
  healthy. Keep the control plane's local `127.0.0.1 remna.arccnet.space` override: three
  consecutive applied syncs verified all 16 selected users plus 11 squad inbounds without
  errors or warnings, while public DNS remains available to external clients.
...
- A legacy paid card row may lack `yookassa_payment_id`; those rows cannot be reconciled by
...
- The Connection Units presentation keeps the established ArcVPN cold palette. Its topology uses
