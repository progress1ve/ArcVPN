...
- Germany recovery: seven valid app keys were absent from the v3 panel after the previous incident (including the reported user). Recreate only missing active keys through `XUIClient.add_client()` on inbound `1`, preserve expiry and traffic limit, then update `client_uuid` in app DB. Verification after repair: 11 active app keys = 11 v3 panel clients. Before repair, make a dated copy under `/root/ArcVPN/backup/deploy/`; never edit `/etc/x-ui/x-ui.db` manually.
