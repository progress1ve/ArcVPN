...
- Причина неработавшего освобождения устройства уточнена по production-логам: Happ удаляет query-параметр `?device=` при последующих обновлениях. Идентификатор перенесён в устойчивый путь `/sub/<sub_id>/<device_token>?format=plain`. Неизвестный, отозванный и вышедший за лимит token никогда не получает рабочие inbound.
