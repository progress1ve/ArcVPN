...
- Историческая привязка Happ Provider ID к `sub.arccnet.space` отменена владельцем 2026-09-01. Не возвращать `providerid` в import URL, комментарии или HTTP-заголовки. Для видимого порядка сервер явно передаёт `subscriptions-sort-type: without`; отдельный Auto JSON-профиль отвечает за выбор по задержке.
