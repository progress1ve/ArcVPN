...
<section class="page">
  ...
  {#if loading}<p>Загружаем каталог…</p>{:else}
    <div class="route">{#each profiles as profile,index}<article class:disabled={!profile.enabled}>
      ...
      <div class="toggles"><label><input type="checkbox" bind:checked={profile.enabled}/><span>{profile.enabled?'Отдельно':'Скрыт'}</span></label><label><input type="checkbox" bind:checked={profile.include_in_auto}/><span>В автовыборе</span></label></div>
...
<style>.page{display:grid;gap:20px;max-width:1100px}.page>header span{color:#80c9f4;font-size:10px;font-weight:900;letter-spacing:.15em}.page h2{margin:8px 0 6px;font-size:30px}.page p{margin:0;color:#7890a5}.route{display:grid;gap:8px}.route article{display:grid;grid-template-columns:48px minmax(0,1fr) 210px 82px;align-items:center;gap:14px;padding:13px 14px;border:1px solid rgba(155,217,255,.1);border-radius:17px;background:#10141c}.route article.disabled{opacity:.72}.route article>b{display:grid;place-items:center;width:38px;height:38px;border-radius:12px;background:#14283a;color:#9bd9ff;font-family:monospace}.route article>div{display:grid;gap:4px}.route article>div.toggles{display:flex;flex-wrap:wrap;gap:9px}.route input[type=text],.route article>div input{border:0;border-bottom:1px solid transparent;background:transparent;color:#edf7ff;font:700 14px inherit;outline:0}.route article>div input:focus{border-color:#80c9f4}.route small{color:#667b8d;font-size:10px}.route label{display:flex;align-items:center;gap:6px;color:#8da1b2;font-size:10px}.route nav{display:flex;gap:5px}.route nav button{width:36px;height:36px;border:0;border-radius:11px;background:#172535;color:#bde7ff}.route nav button:disabled{opacity:.25}footer{display:flex;align-items:center;justify-content:space-between}footer button{min-height:46px;border:0;border-radius:14px;padding:0 20px;background:#9bd9ff;color:#07111d;font-weight:900}@media(max-width:760px){.route article{grid-template-columns:40px 1fr 72px}.route article>div.toggles{grid-column:2/4}.route nav{grid-column:2/4}}</style>
