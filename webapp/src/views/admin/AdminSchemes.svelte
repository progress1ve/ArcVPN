<script>
  ...
  function rebuildGraph() {
    ...
    nodes.set([
      flowNode('gateway', 'Gateway', 'домен подписки', 30, 215, 'gateway', '⌂', { entry: true }),
      flowNode('selector', 'Автовыбор', 'leastLoad · main only', 285, 215, 'selector', '⌘'),
      flowNode('main-pool', 'Входные ноды', `${available}/${items.length} доступны · ${online} онлайн`, 550, 90, available ? 'node' : 'node offline', '▣', { badge: available ? 'online' : 'offline', offline: !available }),
      flowNode('cdn', 'CDN fallback', lteEdges.map((item) => item.country_code).filter(Boolean).join(' + ') || 'NL + DE', 550, 340, 'cdn', '☁', { badge: 'скрытый' }),
      flowNode('internet', 'Internet', 'прямой выход', 825, 215, 'internet', '◎', { exit: true })
    ])
    graphEdges.set([
...
<section class="page">
  ...
  <section class="workspace">
    ...
    <div class="board">
      ...
      <div class="canvas"><SvelteFlow {nodes} edges={graphEdges} {nodeTypes} fitView minZoom={0.55} maxZoom={1.65} nodesConnectable={true} elementsSelectable={true} onconnect={connectBlocks} onnodeschange={() => draftChanged = true} onedgeschange={() => draftChanged = true}><Background variant={BackgroundVariant.Dots} gap={20} size={1} /><MiniMap pannable zoomable /><Controls /></SvelteFlow></div>
