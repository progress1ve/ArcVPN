...
<section class="page">
  ...
  <section class="workspace">
    <aside><small>ПАЛИТРА БЛОКОВ</small><p>Типы маршрутов ArcVPN. Создание и публикация черновика появятся отдельным безопасным этапом.</p>{#each palette as item}<button class={item[1]}><i></i>{item[0]}</button>{/each}<footer>Gateway и Интернет — системные блоки, их нельзя удалить.</footer></aside>
    <div class="board">
      <header><b>{$nodes.length} блоков <i>·</i> {$graphEdges.length} связей</b><span class:bad={mainNodes.some((node) => !node.connected)}>{mainNodes.some((node) => !node.connected) ? 'Есть недоступные ноды' : 'Схема готова'}</span></header>
      <div class="canvas"><SvelteFlow {nodes} edges={graphEdges} fitView minZoom={0.45} maxZoom={1.8} nodesConnectable={false} elementsSelectable={true}><Background variant={BackgroundVariant.Dots} gap={18} size={1} /><MiniMap pannable zoomable /><Controls /></SvelteFlow></div>
      <footer><i></i><span><b>Проверка схемы</b><small>{mainNodes.some((node) => !node.connected) ? 'Недоступные main-ноды исключаются; CDN остаётся аварийным маршрутом.' : 'Main доступны: скрытый CDN не расходует трафик.'}</small></span><em>{activeScheme}</em></footer>
    </div>
  </section>
</section>

<style>
.page{display:grid;gap:22px}.page>header{display:flex;align-items:end;justify-content:space-between;gap:30px}.page>header span,aside>small{color:#80c9f4;font-size:10px;font-weight:900;letter-spacing:.14em}h2{margin:7px 0 5px;font-size:30px}p{margin:0;color:#7890a5}.page>header nav{display:flex;max-width:54%;gap:6px;overflow:auto}.page>header nav button{padding:9px 12px;border:1px solid #1c2a38;border-radius:12px;background:#0c131d;color:#7890a5;white-space:nowrap}.page>header nav button.active{border-color:#37617f;background:#14283a;color:#bde7ff}.workspace{display:grid;grid-template-columns:190px minmax(0,1fr);min-height:650px;overflow:hidden;border:1px solid #1d2b39;border-radius:24px;background:#08111a}.workspace>aside{display:flex;flex-direction:column;gap:7px;padding:18px 12px;border-right:1px solid #1d2b39;background:#0d1621}.workspace>aside p{margin:4px 2px 8px;font-size:10px;line-height:1.45}.workspace>aside button{display:flex;align-items:center;gap:9px;min-height:42px;padding:0 10px;border:1px solid #213140;border-radius:11px;background:#111d29;color:#b4c5d2;text-align:left}.workspace>aside button>i{width:10px;height:10px;border-radius:3px;background:#55d5ac}.workspace>aside .transit>i{background:#4bb7ef}.workspace>aside .cdn>i{background:#ef9b4b}.workspace>aside .warp>i{background:#9d7bea}.workspace>aside .proxy>i{background:#d66bd1}.workspace>aside .block>i{background:#7b8792}.workspace>aside footer{margin-top:auto;color:#607486;font-size:9px;line-height:1.45}.board{display:grid;grid-template-rows:45px 1fr 58px;min-width:0}.board>header{display:flex;align-items:center;justify-content:space-between;padding:0 16px;border-bottom:1px solid #1d2b39}.board>header b{font-size:10px}.board>header b i{color:#557087;font-style:normal}.board>header span{padding:6px 9px;border-radius:9px;background:#123126;color:#78deb0;font-size:9px}.board>header span.bad{background:#352026;color:#ff9da7}.canvas{height:545px;background:#08111a}.board>footer{display:flex;align-items:center;gap:10px;padding:0 16px;border-top:1px solid #1d2b39}.board>footer>i{width:9px;height:9px;border-radius:50%;background:#61d8a5}.board>footer span{display:grid;flex:1}.board>footer small{color:#6f8497;font-size:9px}.board>footer em{color:#80c9f4;font-size:9px;font-style:normal}
