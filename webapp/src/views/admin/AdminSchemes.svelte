<script>
  ...
  function rebuildGraph() {
    ...
    nodes.set([
      flowNode('gateway', 'Gateway', 'клиентский вход', 35, 230, 'gateway'),
      flowNode('selector', 'Автовыбор', 'leastLoad · main only', 285, 230, 'selector'),
      flowNode('main-pool', 'Main-ноды', `${available}/${items.length} доступны · ${online} онлайн`, 555, 115, available ? 'node' : 'node offline'),
      flowNode('cdn', 'Скрытый CDN fallback', lteEdges.map((item) => item.country_code).filter(Boolean).join(' + ') || 'NL + DE', 555, 365, 'cdn'),
      flowNode('internet', 'Интернет', 'итоговый egress', 825, 230, 'internet')
    ])
    graphEdges.set([
...
<section class="page">
  ...
  <section class="workspace">
    ...
    <div class="board">
      ...
      <div class="canvas"><SvelteFlow {nodes} edges={graphEdges} fitView minZoom={0.55} maxZoom={1.65} nodesConnectable={true} elementsSelectable={true} onconnect={connectBlocks} onnodeschange={() => draftChanged = true} onedgeschange={() => draftChanged = true}><Background variant={BackgroundVariant.Dots} gap={20} size={1} /><MiniMap pannable zoomable /><Controls /></SvelteFlow></div>
...
/* Quiet operations canvas: compact, warm and topology-first. */
.page{gap:18px;color:#f3eeee}.page>header{align-items:center}.page>header span,.workspace>aside>small{color:#d99aa5}.page>header h2{font-size:27px;letter-spacing:-.035em}.page>header p{color:#8e8185;font-size:12px}.scheme-select{display:grid;min-width:270px;gap:6px}.scheme-select small{color:#776b6f;font-size:9px;text-transform:uppercase;letter-spacing:.12em}.scheme-select select{height:42px;border:1px solid #34282b;border-radius:11px;padding:0 36px 0 13px;background:#151012;color:#eee5e7;font:700 12px inherit;outline:none}.workspace{grid-template-columns:178px minmax(0,1fr);min-height:614px;border-color:#302629;border-radius:20px;background:#0d0a0b}.workspace>aside{padding:16px 11px;border-color:#302629;background:#141012}.workspace>aside p{color:#7e7276}.workspace>aside button{min-height:39px;border-color:#332a2d;border-radius:9px;background:#1b1517;color:#bcaeb1}.workspace>aside button:hover{border-color:#5a4248;background:#21191c}.workspace>aside button>b{color:#796c70}.workspace>aside footer{color:#6d6266}.board{grid-template-rows:43px 500px auto}.board>header{border-color:#302629;background:#120e10}.board>header>b,.board>header>b strong{color:#bfb2b5}.board>header span{background:#183025;color:#75d7a5}.board>header span.bad{background:#3b2027;color:#f5a0ad}.board>header button{border-color:#34292c;background:#191315;color:#9d8f93}.canvas{height:500px;background:#0d0a0b}.findings{border-color:#302629;background:#130f11}.findings-head{color:#e7dcdf}.findings-head span{color:#796d70}.findings-list article.good{border-color:#294538;background:#14251e}.findings-list article{border-color:#543039;background:#28191e}
:global(.svelte-flow){--xy-background-color:#0d0a0b;--xy-node-border:1px solid #443438;--xy-node-border-radius:11px;--xy-node-background-color:#1a1416;--xy-node-color:#eee7e9;--xy-edge-stroke:#b88991;--xy-edge-stroke-width:1.5;--xy-controls-button-background-color:#181315;--xy-controls-button-color:#c8b9bd;--xy-controls-button-border-color:#382c2f;--xy-minimap-background-color:#161113}:global(.svelte-flow__node){width:180px;min-height:56px;padding:10px 12px;border-color:#443438;line-height:1.35;box-shadow:0 10px 24px #0007;font-size:10px}:global(.svelte-flow__node.gateway){border-color:#75535c}:global(.svelte-flow__node.selector){border-color:#4d7f6c}:global(.svelte-flow__node.cdn){border-color:#6a568b}:global(.svelte-flow__node.internet){border-color:#6d555c}:global(.svelte-flow__node.offline){border-color:#7d404b}:global(.svelte-flow__edge path){stroke:#aa7b84}:global(.svelte-flow__edge.fallback path){stroke:#8f73c7;stroke-dasharray:7 6}:global(.svelte-flow__edge-text){fill:#a99a9e;font-size:8px}:global(.svelte-flow__edge-textbg){fill:#171214;fill-opacity:1}:global(.svelte-flow__minimap){width:145px!important;height:92px!important;border-color:#403236;border-radius:9px;background:#161113!important}:global(.svelte-flow__controls){border-color:#403236;border-radius:8px}:global(.svelte-flow__attribution){font-size:7px;opacity:.35}
