<script>
  ...
  const uptime = value => { const seconds=Number(value||0); if(!seconds)return 'нет данных'; const days=Math.floor(seconds/86400); const hours=Math.floor(seconds%86400/3600); return `${days} д ${hours} ч` }
...
<section class="page" aria-labelledby="health-title">
  ...
  <div class:danger={criticalCount > 0} class:warning={!criticalCount && incidents.length} class="score" role="status" aria-live="polite"><b aria-hidden="true">{incidents.length ? '!' : '✓'}</b><span><em>{criticalCount ? 'КРИТИЧЕСКОЕ СОСТОЯНИЕ' : incidents.length ? 'ТРЕБУЕТСЯ ВНИМАНИЕ' : 'СИСТЕМА В НОРМЕ'}</em><h3>{incidents.length ? `${incidents.length} активных условий` : 'Активных предупреждений нет'}</h3><p>{activeNodes.length ? `${activeNodes.filter((node) => node.connected).length}/${activeNodes.length} рабочих нод подключены` : 'Ноды не обнаружены'} · неизвестные значения не считаются здоровыми.</p></span><div class="score-metrics"><span><b>{data?.remnawave?.online_users?.length || 0}</b><small>пользователей онлайн</small></span><span><b>{uptime(data?.system?.uptime_seconds)}</b><small>uptime Poland</small></span></div></div>
...
.score-metrics{display:flex!important;gap:18px}.score-metrics span{display:grid;gap:3px}.score-metrics span+span{padding-left:18px;border-left:1px solid rgba(155,217,255,.12)}.score-metrics span b{font-size:20px}@media(max-width:560px){.score-metrics{flex-wrap:wrap}.score-metrics span+span{padding-left:0;border-left:0}}
</style>
