<script>
  ...
  $: incidents = [
    ...
    ...nodes.filter((node) => !node.connected).map((node) => ({ severity: 'critical', title: `Нода «${node.name}» недоступна`, detail: node.last_status_message || 'Нет соединения с RemnaNode', object: node.address })),
  ...
  $: services = [
    ...
    { name: 'Диск', ok: diskPct < 85, detail: diskPct ? `${diskPct}% занято` : 'Нет данных' },
  ]
...
<section class="page" aria-labelledby="health-title">
  ...
  <div class:danger={criticalCount > 0} class:warning={!criticalCount && incidents.length} class="score" role="status" aria-live="polite"><b aria-hidden="true">{incidents.length ? '!' : '✓'}</b><span><em>{criticalCount ? 'КРИТИЧЕСКОЕ СОСТОЯНИЕ' : incidents.length ? 'ТРЕБУЕТСЯ ВНИМАНИЕ' : 'СИСТЕМА В НОРМЕ'}</em><h3>{incidents.length ? `${incidents.length} ${incidents.length === 1 ? 'активное событие' : 'активных события'}` : 'Активных событий нет'}</h3><p>{nodes.length ? `${nodes.filter((node) => node.connected).length}/${nodes.length} нод подключены` : 'Ноды не обнаружены'} · оценка строится по подтверждённым сигналам без условного процента.</p></span><div><b>{data?.remnawave?.online_users?.length || 0}</b><small>пользователей онлайн</small></div></div>
  ...
  <div class="rows">
    ...
    {:else if view === 'nodes'}
      {#if nodes.length}{#each nodes as node}<article class:ok={node.connected}><i></i><span><b>{node.name}</b><p>{node.connected ? `${node.users_online || 0} онлайн · RAM ${node.memory_used_pct || 0}% · ${node.traffic_used_gb || 0} ГБ` : node.last_status_message || 'RemnaNode недоступна'}</p><small>{node.address}</small></span></article>{/each}{:else}<div class="empty"><b>Ноды не обнаружены</b><p>Проверьте подключение Remnawave и повторите сбор данных.</p></div>{/if}
