<script>
  ...
  const appCatalog = {
    happ: {
      label: 'Happ',
      art: `${asset}/connect-happ-phone-v2.png`,
      stores: {
        iphone: 'https://apps.apple.com/ru/app/happ-proxy-utility/id6783623643',
  ...
  $: customMonthlyReferenceRub = customTariffPrice(plans, 1, customDevices, customLteGb)
  $: customComparisonRub = customMonths > 1 ? customMonthlyReferenceRub * customMonths : 0
  $: customSavingsRub = Math.max(0, customComparisonRub - customBaseRub)
      ...
      {:else if connectOpen}
        <section class="screen connect-page" aria-label="Подключение VPN">
              ...
              <div class="connect-device-grid">
                {#each devicesList as item}
                  <button on:click={() => chooseDevice(item.id)}>
                    <i>{#if item.id === 'linux'}<img class="linux-device-icon" src={`${asset}/linux-tux.svg`} alt="" />{:else}<DeviceIcon name={item.icon} size={27} />{/if}</i>
          ...
          {:else if connectStage === 'app'}
            ...
            <button class="connect-secondary-action" on:click={importSelectedApp}>Импортировать в {currentConnectApp.label}</button>
          {:else}
...
<style>
  ...
  .telegram-login { width: 100%; min-height: 52px; display: flex; align-items: center; justify-content: center; gap: 9px; margin-top: 28px; border: 1px solid rgba(143,215,251,.42); border-radius: 16px; color: #07131d; background: linear-gradient(125deg,#a9e2ff,#69bdec); font-size: 12px; font-weight: 850; box-shadow: 0 16px 40px rgba(55,157,217,.13); }
  .telegram-login:hover { border-color: #d8f2ff; filter: brightness(1.04); }
  ...
  .payment-method-backdrop{position:fixed;z-index:80;inset:0;display:grid;place-items:end center;padding:16px;background:rgba(1,6,12,.68);backdrop-filter:blur(12px)}.payment-method-sheet{box-sizing:border-box;width:min(100%,540px);padding:24px;border:1px solid rgba(180,220,250,.1);border-radius:32px;background:linear-gradient(155deg,#101923,#0a111a 68%);box-shadow:0 30px 100px rgba(0,0,0,.55)}.payment-method-sheet header{display:flex;align-items:center;justify-content:space-between}.payment-method-sheet h2{margin:0;color:#fff;font-size:24px;letter-spacing:-.035em}.payment-method-sheet header button{display:grid;place-items:center;width:44px;height:44px;border:0;border-radius:50%;background:rgba(255,255,255,.06);color:#c1cedb;font-size:25px}.payment-options{display:grid;gap:11px;margin:24px 0 18px}.payment-options>button{display:grid;grid-template-columns:52px 1fr 24px;align-items:center;gap:12px;min-height:78px;padding:12px 16px;border:1px solid rgba(174,211,241,.1);border-radius:24px;background:rgba(255,255,255,.025);color:#fff;text-align:left;transition:.2s}.payment-options>button.active{border-color:#83cdf7;background:rgba(105,191,240,.08);box-shadow:inset 0 0 0 1px rgba(131,205,247,.08)}.payment-options>button>i{display:grid;place-items:center;width:48px;height:48px;border-radius:50%;background:rgba(159,213,248,.09);color:#a9dcfb;font-size:14px;font-style:normal;font-weight:900}.payment-options span{display:flex;flex-direction:column;gap:4px}.payment-options b{font-size:16px}.payment-options small,.autorenew small{color:#9cafc0;font-size:10px}.payment-options em{display:grid;place-items:center;width:24px;height:24px;border-radius:50%;color:transparent;font-style:normal;transition:.2s}.payment-options>button.active em{background:#96d8ff;color:#07111b;box-shadow:inset 0 1px rgba(255,255,255,.58),0 6px 18px rgba(91,183,235,.18)}.autorenew{box-sizing:border-box;width:100%;display:flex;align-items:center;gap:12px;min-height:58px;padding:10px 14px;border:1px solid rgba(166,211,244,.13);border-radius:18px;background:rgba(139,196,235,.055);color:#f5f9fd;text-align:left;opacity:1;transition:.2s}.autorenew:hover{background:rgba(139,196,235,.085)}.autorenew>i{display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:rgba(151,201,237,.12);color:transparent;font-style:normal;transition:.2s}.autorenew>i.checked{background:#96d8ff;color:#07111b;box-shadow:inset 0 1px rgba(255,255,255,.58),0 6px 18px rgba(91,183,235,.16)}.autorenew span{display:flex;flex-direction:column;gap:3px}.autorenew b{font-size:14px}.method-confirm{width:100%;min-height:58px;margin-top:20px;border:0;border-radius:20px;background:linear-gradient(125deg,#b6e7ff,#6bc0ef);color:#07131d;font-size:14px;font-weight:900}.payment-method-sheet>p{margin:14px 12px 0;color:#8297aa;font-size:9px;line-height:1.5;text-align:center}.payment-method-sheet a{color:#9bd9ff}
  ...
  .linux-device-icon { width: 28px; height: 28px; object-fit: contain; filter: brightness(0) invert(1); }
