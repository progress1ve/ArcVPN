<script>
  ...
  $: visibleUsers = (data?.recent_users || []).filter((u) => userFilter === 'all' || (userFilter === 'online' ? Number(u.online_devices) > 0 : userFilter === 'inactive' ? !u.active : true)).sort((a,b) => userFilter === 'top' ? Number(b.paid_rub)-Number(a.paid_rub) : 0)
  ...
  <main>
    ...
    {:else if active === 'network'}
      <section class="workspace-section">
        ...
        <div class="network-grid">{#each data.remnawave?.nodes || [] as node}<article class:offline={!node.connected}><div class="node-name"><i class:offline={!node.connected}></i><div><b>{node.name}</b><span>{node.address} · {node.inbounds?.length || 0} inbound</span></div><em class:bad={!node.connected}>{node.connected ? 'ONLINE' : 'OFFLINE'}</em></div><p>{node.inbounds?.map(i=>`${String(i.network||i.type||'').toUpperCase()}:${i.port}`).join(' · ') || 'Нет активных inbound'}</p><strong>{num(node.users_online)}</strong><small>пользователей онлайн · передано {node.traffic_used_gb} ГБ</small><div class="node-day"><span>RAM {node.memory_used_pct}%</span><span>load {node.load_1m ?? '—'}</span><span>↓ {mbps(node.rx_bps)}</span><span>↑ {mbps(node.tx_bps)}</span><span>uptime {Math.floor(Number(node.xray_uptime_seconds||0)/3600)} ч</span></div><div class="bar"><i style={`width:${Math.min(100,Number(node.memory_used_pct||0))}%`}></i></div></article>{/each}</div>
