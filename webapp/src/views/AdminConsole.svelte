    ...
    {:else}
      ...
      <div class="overview-grid">
        <section class="panel node-panel">
          <div class="panel-head"><div><span>Инфраструктура</span><h2>Узлы сети</h2></div><button on:click={() => openSection('nodes')}>Все узлы <ArcIcon name="arrow" size={16} /></button></div>
...
<style>
  ...
  @media(max-width:900px){.console{grid-template-columns:76px 1fr}aside{padding:20px 10px}.brand span,nav span,.owner span{display:none}.brand{justify-content:center;padding-inline:0}nav button{justify-content:center;padding:0}.owner{justify-content:center;background:transparent}.metrics,.referral-metrics{grid-template-columns:1fr 1fr}.referral-leaders{grid-template-columns:1fr 1fr}.node-panel,.queue{grid-column:1/-1}.device-control>div:last-child{grid-template-columns:1fr 1fr}main{width:calc(100% - 32px);padding-top:28px}}
  ...
  .referral-metrics article{border:1px solid rgba(255,255,255,.045);border-radius:15px;background:#0c1118}
  ...
  @media(max-width:560px){.console{display:block}.console>aside>nav{height:48px;margin-top:0;overflow-x:auto;overflow-y:hidden}.device-control>div:last-child{grid-template-columns:1fr}}
</style>
