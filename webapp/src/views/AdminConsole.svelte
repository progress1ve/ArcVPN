  ...
  <main>
    ...
    {:else if active === 'payments'}
      <section class="panel records"><div class="panel-head"><div><span>Деньги и заказы</span><h2>Платежи</h2></div></div>
        ...
        <div class="record-list">{#each visiblePayments as payment}<article><div><b>{rub(Number(payment.amount_cents || 0) / 100)}</b><small>@{payment.username || payment.telegram_id} · {payment.tariff_name || payment.payment_type || 'оплата'}{payment.legacy_price ? ' · старый ошибочный заказ' : ''}</small></div><span class:ok={payment.status === 'succeeded' || payment.status === 'paid'}>{payment.status}</span></article>{/each}</div>
      </section>
    {:else if active === 'network'}
      <section class="workspace-section">
        <div class="section-title"><div><span class="eyebrow">Операционный центр</span><h2>Сеть ArcVPN</h2><p>Ноды, процессы и точки входа — из живой телеметрии 3x-ui.</p></div><button class="refresh" on:click={load}><ArcIcon name="pulse" size={18}/>Проверить</button></div>
        <div class="service-strip">
          <article class:ok={data.operations.panel_api_healthy}><i></i><span><b>API панели</b><small>{data.operations.panel_api_healthy ? 'Авторизация работает' : 'Нет связи'}</small></span></article>
          <article class:ok={data.operations.xui_service}><i></i><span><b>3x-ui</b><small>{data.operations.xui_service ? 'Служба запущена' : 'Служба остановлена'}</small></span></article>
          ...
          <article class:ok={data.operations.hysteria_service}><i></i><span><b>Hysteria2</b><small>{data.operations.hysteria_service ? 'Служба запущена' : 'Недоступна'}</small></span></article>
        ...
        <div class="network-grid">{#each data.servers as server}<article class:offline={!server.is_active}><div class="node-name"><i class:offline={!server.is_active}></i><div><b>{server.name}</b><span>{server.provider || 'Провайдер не указан'} · {server.location || server.host}</span></div><em class:bad={!server.agent_online}>{server.agent_online ? (server.health_state || 'healthy') : 'agent offline'}</em></div><p>{server.monthly_cost_rub ? `${server.monthly_cost_rub} ₽/мес` : 'Стоимость не указана'} · заявлено до {server.capacity_mbps || '—'} Мбит/с</p><strong>{server.telemetry_available===false ? '—' : num(server.active_clients)}</strong><small>{server.telemetry_available===false ? 'нет данных о подключениях' : `онлайн · ${server.latency_ms||0} мс · CPU ${server.cpu_pct??'—'}% · RAM ${server.mem_pct??'—'}%`}</small>{#if server.agent_online}<div class="node-day"><span>↓ {mbps(server.net_rx_bps)}</span><span>↑ {mbps(server.net_tx_bps)}</span><span>load {server.load_1m ?? '—'}</span><span>steal {server.cpu_steal_pct ?? '—'}%</span><span>disk {server.disk_used_pct ?? '—'}%</span><span>TCP {num(server.tcp_established)}</span></div>{/if}{#if data.server_health_24h?.[server.host]}<div class="node-day"><span>24ч: {data.server_health_24h[server.host].availability_pct}%</span><span>avg CPU {data.server_health_24h[server.host].avg_cpu_pct ?? '—'}%</span><span>max CPU {data.server_health_24h[server.host].max_cpu_pct ?? '—'}%</span></div>{/if}{#if server.telemetry_available!==false}<div class="bar"><i style={`width:${Math.min(100,(Number(server.active_clients||0)/Math.max(1,Number(server.clients_count||0)))*100)}%`}></i></div>{/if}</article>{/each}</div>
        <section class="inbound-board"><header><div><span class="eyebrow">Точки входа</span><h3>Inbound-контур</h3></div><strong>{data.inbounds?.filter(item=>item.enabled).length || 0}/{data.inbounds?.length || 0} работают</strong></header><div>{#each data.inbounds || [] as inbound}<article><i class:off={!inbound.enabled}></i><span><b>{inbound.name}</b><small>{inbound.location} · {inbound.protocol} · порт {inbound.port}</small></span><em>{num(inbound.clients)} клиентов</em></article>{/each}</div></section>
      </section>
    ...
    {:else}
      ...
      <section class="metrics">
        ...
        <article><span>Сейчас онлайн</span><strong>{num(data.operations.panel_online_total ?? data.activity.online_now)}</strong><small>живые сессии 3x-ui · {num(data.activity.week)} за неделю</small></article>
      </section>
      <div class="grid">
        <section class="panel">
