  ...
  <main class={`section-${active}`}>
    ...
    {:else}
      ...
      <section class="metrics">
        <article><i class="metric-icon blue"><ArcIcon name="users" size={18} /></i><span>Новые пользователи</span><strong>{num(data.users?.[overviewPeriod])}</strong><small>за {periodLabel[overviewPeriod]}</small></article>
        <article><i class="metric-icon green"><ArcIcon name="wallet" size={18} /></i><span>Успешные оплаты</span><strong>{num(data.business?.payments?.[overviewPeriod]?.orders)}</strong><small>{num(data.business?.payments?.[overviewPeriod]?.paying_users)} покупателей</small></article>
        <article><i class="metric-icon amber"><ArcIcon name="signal" size={18} /></i><span>Выручка</span><strong>{rub(data.business?.payments?.[overviewPeriod]?.revenue_rub)}</strong><small>подтверждено · {periodLabel[overviewPeriod]}</small></article>
        <article><i class="metric-icon violet"><ArcIcon name="pulse" size={18} /></i><span>Сейчас онлайн</span><strong>{num(data.remnawave?.online_users?.length || 0)}</strong><small>Remnawave · 3 минуты</small></article>
      </section>
...
<style>
  ...
  /* Readable operations surface. Values and descriptions retain their hierarchy at 100% zoom. */
  :global(body.admin-console-open){background:#0b1220}
  .console{--card:#131c2b;--line:#2b384b;grid-template-columns:248px minmax(0,1fr);background:#0b1220;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;font-size:14px;color:#eaf0f8}
  main{width:min(1480px,calc(100% - 64px));padding:32px 0 64px;scrollbar-width:thin}
  .health{min-height:52px;padding:14px 20px;gap:14px;border:1px solid #285244;border-radius:12px;background:#132820}.health>i{width:8px;height:8px;box-shadow:none}.health div{gap:5px}.health div b{font-size:14px;font-weight:600}.health span{color:#a9c6b8;font-size:12px}.health>strong{padding:6px 10px;border-radius:7px;background:#204336;color:#a0edc9;font-size:12px;font-weight:500}
  .periods{margin:20px 0 0;padding:4px;border-color:#33425a;border-radius:10px;background:#131c2b;overflow:visible}.periods button{min-height:36px;padding:0 18px;border-radius:7px;font-size:13px;font-weight:500;transform:none;color:#a6b4c8}.periods button.active{background:#274e75;color:#e5f3ff;box-shadow:none}
  .metrics{gap:16px;margin:16px 0 24px}.metrics article{position:relative;min-height:132px;padding:22px;display:flex;border:1px solid #2b384b;border-radius:16px;background:#131c2b!important;box-shadow:none}.metrics .metric-icon{position:absolute;right:20px;top:20px;width:38px;height:38px;display:grid;place-items:center;border-radius:11px;font-style:normal}.metric-icon.blue{background:#1c3853;color:#8bcfff}.metric-icon.green{background:#204135;color:#91e8bc}.metric-icon.amber{background:#463925;color:#f7d290}.metric-icon.violet{background:#383151;color:#c6b6fa}.metrics span{padding-right:42px;color:#b5c3d6;font-size:14px;line-height:1.5}.metrics strong{margin:16px 0 8px;font-size:32px;font-weight:600;letter-spacing:-.025em;font-variant-numeric:tabular-nums}.metrics small{color:#a6b4c8;font-size:12px}
  .overview-grid{gap:20px;align-items:stretch}.panel{padding:24px;border:1px solid #2b384b;border-radius:16px;background:#131c2b;box-shadow:none}.panel-head{margin-bottom:20px;gap:14px}.panel-head span{display:none}.panel-head h2{margin:0;font-size:18px;font-weight:600;letter-spacing:-.015em}.panel-head button{font-size:13px;color:#9dd5ff;font-weight:500}.node-panel{grid-column:span 7;order:1}.queue{grid-column:span 5;order:2}.acquisition-panel{grid-column:span 7;order:3}.product-panel{grid-column:span 5;order:4}.traffic-panel{grid-column:span 6;order:5}.rating-panel{grid-column:span 6;order:6}
  .nodes{display:flex;flex-direction:column;gap:0;background:transparent}.nodes article{display:grid;grid-template-columns:minmax(0,1fr) 70px;align-items:center;gap:5px 16px;min-height:62px;padding:14px 0;border-bottom:1px solid #2b384b;border-radius:0;background:transparent}.nodes article:last-child{border-bottom:0}.node-name{grid-row:span 2;min-width:0}.node-name div{min-width:0;gap:6px}.node-name b{font-size:14px;font-weight:500;overflow-wrap:anywhere}.node-name span{font-size:12px;color:#a6b4c8;overflow-wrap:anywhere}.nodes article>strong{margin:0;text-align:right;font-size:20px;font-weight:600}.nodes article>small{font-size:12px;color:#a6b4c8;text-align:right}
  .queue>button{min-height:72px;padding:14px 0;gap:14px;border-color:#2b384b;cursor:pointer}.queue button>i{width:38px;height:38px;border-radius:10px;flex-shrink:0}.queue button b{font-size:14px;font-weight:500}.queue button small{color:#a6b4c8;font-size:12px;line-height:1.5}.queue button>strong{min-width:36px;height:32px;font-size:14px;border-radius:8px;background:#213149}
  .source-grid,.traffic-grid{gap:18px}.source-grid article,.traffic-grid article{padding:8px 0;border-radius:0;background:transparent;gap:10px}.source-grid b,.traffic-grid b{font-size:28px;font-weight:600}.source-grid span,.traffic-grid span,.traffic-grid small{font-size:13px;color:#a6b4c8;line-height:1.5}
  .rating-summary{gap:16px}.rating-summary article{padding:8px 0;border-radius:0;background:transparent}.rating-summary b{font-size:24px;font-weight:600}.rating-summary span,.rating-distribution,.rating-responses small,.product-list small,.empty-copy{font-size:13px;color:#a6b4c8;line-height:1.5}.rating-distribution{flex-wrap:wrap}.product-list article,.rating-responses article{padding:14px 0;border-color:#2b384b}.product-list b,.rating-responses b{font-size:14px;font-weight:500}.product-list strong{background:#233c55;font-size:14px;border-radius:8px}
  @media(max-width:1250px){.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.node-panel,.queue,.acquisition-panel,.product-panel,.traffic-panel,.rating-panel{grid-column:1/-1}}
  @media(max-width:1100px){.console{grid-template-columns:72px minmax(0,1fr)}main{width:calc(100% - 40px)}}
  @media(max-width:560px){.console{display:block}.console>main{width:calc(100% - 32px);padding:24px 0 calc(100px + env(safe-area-inset-bottom,0px))}.metrics{gap:12px}.metrics article{min-height:120px;padding:16px}.metrics .metric-icon{position:static;width:30px;height:30px;margin-bottom:12px}.metrics span{padding:0;font-size:13px}.metrics strong{font-size:26px}.metrics small{font-size:12px}.panel{padding:18px}.panel-head{align-items:center}.health>strong{display:none}.health{padding:14px}.periods{width:100%}.periods button{flex:1;padding:0 12px}.traffic-grid{grid-template-columns:1fr}.source-grid{grid-template-columns:1fr}.source-grid article{display:flex;align-items:baseline;gap:14px}.queue button b{font-size:13px}}
</style>
