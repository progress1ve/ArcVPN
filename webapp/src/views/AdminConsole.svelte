  ...
  <main>
    ...
    {:else if active === 'network'}
      <section class="workspace-section">
        ...
        <div class="network-grid">{#each data.servers as server}<article class:offline={!server.is_active}><div class="node-name"><i class:offline={!server.is_active}></i><div><b>{server.name}</b><span>{server.provider || 'Провайдер не указан'} · {server.location || server.host}</span></div><em class:bad={server.health_state==='offline'}>{server.health_state || 'unknown'}</em></div><p>{server.monthly_cost_rub ? `${server.monthly_cost_rub} ₽/мес` : 'Стоимость не указана'} · до {server.capacity_mbps || '—'} Мбит/с</p><strong>{server.telemetry_available===false ? '—' : num(server.active_clients)}</strong><small>{server.telemetry_available===false ? 'нет данных о подключениях' : `онлайн · ${server.latency_ms||0} мс · CPU ${server.cpu_pct??'—'}% · RAM ${server.mem_pct??'—'}%`}</small>{#if data.server_health_24h?.[server.host]}<div class="node-day"><span>24ч: {data.server_health_24h[server.host].availability_pct}%</span><span>avg {data.server_health_24h[server.host].avg_latency_ms ?? '—'} мс</span><span>max CPU {data.server_health_24h[server.host].max_cpu_pct ?? '—'}%</span></div>{/if}{#if server.telemetry_available!==false}<div class="bar"><i style={`width:${Math.min(100,(Number(server.active_clients||0)/Math.max(1,Number(server.clients_count||0)))*100)}%`}></i></div>{/if}</article>{/each}</div>
