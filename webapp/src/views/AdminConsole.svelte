<script>
  ...
  import { fetchAdminAccess, fetchAdminOverview, loginAdmin, logoutAdmin, runAdminNodeDiagnostic, fetchAdminSupportThreads, fetchAdminSupportThread, sendAdminSupportReply } from '../lib/api.js'
  ...
  const nav = [
    ...
    ['schemes', 'signal', 'Схемы подключений', 'overview.read'], ['nodes', 'devices', 'Ноды', 'overview.read'],
  ...
  async function signOut() { await logoutAdmin(); access = null; data = null; error = 'auth' }
...
<style>
  ...
  @media(max-width:900px){.console{grid-template-columns:76px 1fr}aside{padding:20px 10px}.brand span,nav span,.owner span{display:none}.brand{justify-content:center;padding-inline:0}nav button{justify-content:center;padding:0}.owner{justify-content:center;background:transparent}.metrics,.referral-metrics{grid-template-columns:1fr 1fr}.referral-leaders{grid-template-columns:1fr 1fr}.grid{grid-template-columns:1fr}main{width:calc(100% - 32px);padding-top:28px}}
