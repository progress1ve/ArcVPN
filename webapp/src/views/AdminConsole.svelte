  ...
  <main>
    ...
    {:else if active === 'network'}
      <section class="workspace-section">
        ...
        <div class="network-grid">{#each data.servers as server}<article class:offline={!server.is_active}><div class="node-name"><i class:offline={!server.is_active}></i><div><b>{server.name}</b><span>{server.is_active ? `Работает${server.latency_ms ? ` · ${server.latency_ms} мс` : ''}` : server.telemetry_available===false ? 'Телеметрия недоступна' : 'Недоступен'}</span></div></div><strong>{server.telemetry_available===false ? '—' : num(server.active_clients)}</strong><small>{server.telemetry_available===false ? 'нет данных о подключениях' : `онлайн из ${num(server.clients_count)} клиентов`}</small>{#if server.telemetry_available!==false}<div class="bar"><i style={`width:${Math.min(100,(Number(server.active_clients||0)/Math.max(1,Number(server.clients_count||0)))*100)}%`}></i></div>{/if}</article>{/each}</div>
