...
The command prompts securely. Use `-Action List` to show aliases only and `-Action Remove -Alias <name>` to remove one exact vault entry. Never commit `.secrets`, export decrypted values, or copy the vault to production.
