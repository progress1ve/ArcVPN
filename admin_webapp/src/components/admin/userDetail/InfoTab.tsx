...
export function InfoTab(props: InfoTabProps) {
  ...
  const arcReferral = user.referral as typeof user.referral & {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl bg-dark-800/50 p-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">
          ...
          <div>
            <div className="text-lg font-bold text-dark-100">{arcReferral.paid_referrals ?? 0}</div>
            <div className="text-xs text-dark-500">Оплатили</div>
          </div>
          <div>
            <div className="text-lg font-bold text-dark-100">{arcReferral.earned_days ?? 0}</div>
            <div className="text-xs text-dark-500">Дней начислено</div>
          </div>
          <div>
            {editingReferralCommission ? (
