...
export function ReferralsTab({ user, userId, onUserRefresh }: ReferralsTabProps) {
  ...
  const canEdit = usePermissionStore((state) => state.hasPermission('users:referral'));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          label="Оплатили"
          value={arcReferral.paid_referrals ?? 0}
          icon={<BanknotesIcon className="h-5 w-5" />}
          tone="neutral"
        />
        <StatCard
          label="Дней начислено"
          value={arcReferral.earned_days ?? 0}
