...
export const adminUsersApi = {
  ...
  getUsers: async (
    ...
    const response = await getArcUsers(params);
    return {
      users: (response.users || []).map(userListItem),
      total: Number(response.total || 0),
      offset: Number(response.cursor || params.offset || 0),
  ...
  getStats: async (): Promise<UsersStatsResponse> => {
    return usersStats() as Promise<UsersStatsResponse>;
  },

  // Get user detail
  getUser: async (userId: number): Promise<UserDetailResponse> => {
    return userDetail(userId) as Promise<UserDetailResponse>;
  },

  // Get user by telegram ID
  getUserByTelegram: async (telegramId: number): Promise<UserDetailResponse> => {
    return userDetail(telegramId) as Promise<UserDetailResponse>;
  },
  ...
  getReferrals: async (userId: number, offset = 0, limit = 50): Promise<UsersListResponse> => {
    void offset;
    void limit;
    return referralUsers(userId) as Promise<UsersListResponse>;
  },
  ...
  getSyncStatus: async (
    ...
    void subscriptionId;
    return syncStatus(userId) as Promise<PanelSyncStatusResponse>;
  },
  ...
  getNodeUsage: async (userId: number, subscriptionId?: number): Promise<UserNodeUsageResponse> => {
    void userId;
    void subscriptionId;
    return { items: [], categories: [], period_days: 30 };
  },
  ...
  getUserActivity: async (
    ...
    void limit;
    void types;
    return userActivity(userId) as Promise<UserActivityResponse>;
  },
  ...
  getUserDevices: async (
    ...
    void subscriptionId;
    return userDevices(userId) as Promise<{
      devices: {
        hwid: string;
        platform: string;
        device_model: string;
  ...
  getUserGifts: async (userId: number): Promise<AdminUserGiftsResponse> => {
    void userId;
    return { sent: [], received: [], sent_total: 0, received_total: 0 };
  },
};
