    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-1.5">
          ...
          <span className="text-xs text-dark-400 lg:hidden">
      ...
      <div className="hidden min-w-[250px] grid-cols-2 gap-5 lg:grid">
        <div>
          <div className="text-[11px] uppercase tracking-wide text-dark-500">Основной трафик</div>
          <div className="mt-1 font-mono text-sm text-dark-100">{(user.main_used_gb ?? user.traffic_used_gb).toFixed(1)} ГБ</div>
        </div>
        <div>
          <div className="text-[11px] uppercase tracking-wide text-dark-500">LTE-трафик</div>
          <div className="mt-1 font-mono text-sm text-dark-100">{(user.lte_used_gb ?? 0).toFixed(1)} / {user.lte_quota_gb ?? 0} ГБ</div>
        </div>
      </div>

      <div className="hidden min-w-[120px] sm:block">
        {Number(user.online_devices || 0) > 0 ? (
          <div><div className="flex items-center gap-2 text-sm font-medium text-success-400"><span className="h-2 w-2 rounded-full bg-success-400" />Онлайн</div><div className="mt-1 max-w-[140px] truncate text-xs text-dark-500">{user.online_node || `${user.online_devices} устр.`}</div></div>
        ) : <div className="text-sm text-dark-500">Не в сети</div>}
      </div>
      ...
      <div className="mb-4 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            <option value="main_traffic">По основному трафику</option>
            <option value="lte_traffic">По LTE-трафику</option>
