...
function NodeCard({ node, onRestart, onToggle, isLoading, canManage }: NodeCardProps) {
      ...
      {canManage && (
        <div className="flex gap-2">
          ...
          >
            ...
            {node.is_disabled
              ? t('adminDashboard.nodes.enable')
              : t('adminDashboard.nodes.disable')}
          </button>
          <button
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              {(showAllNodes ? stats.nodes.nodes : stats.nodes.nodes.slice(0, 3)).map((node) => (
                <NodeCard
                  ...
                  canManage={hasPermission('servers:write')}
                />
              ))}
            </div>
